...
class absenteeism_model():
        ...
        def load_and_clean_data(self, data_file):
            ...
            # we need this lines so we can use it in the next functions (Llama al self.scaler para el scaler file y despues al metodo trans)
            columns_to_omit = ['Reason_1', 'Reason_2', 'Reason_3', 'Reason_4','Education']
            columns_to_scale = [x for x in df.columns.values if x not in columns_to_omit]
            absenteeism_scaler = CustomScaler(columns_to_scale)
            absenteeism_scaler.fit(df)
            self.data = absenteeism_scaler.transform(df)
